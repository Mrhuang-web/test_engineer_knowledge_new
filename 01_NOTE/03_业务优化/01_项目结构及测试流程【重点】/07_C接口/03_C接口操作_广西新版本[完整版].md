...
以广西为例【接入广西lsc】
	C模拟器环境选择【目前是在10.1.203.38这个服务上】
		...
		其次需要修改config.py文件
			需要修改参数
				...
				http：这个是C模拟器的端口【也需要看目前服务下，是否已经被应用了；即在10.1.203.38这个服务上  -- lsof -i:端口查看】
					2900
				数据库mysql【改成】：
					MYSQL_CONN_STR = "mysql+pymysql://root:G$SGp!8L3O@10.1.203.38:3306/cinterdb_400?charset=utf8&autocommit=true"
		...
		紧接着运行启动即可
			可以看nohup日志【日志主要看C模拟器启动的请求、C模拟器数据响应情况】
```
...
```
说明：
	这里主要是把站点、机房、设备、测点的结构写道中间库
	用于后面站点映射时候，映射过去【即把整个结构迁过去】
	目前采用的是400版本、同时数据库直接用现有的，并没有执行脚本生成新的数据库表【因为有问题】
	存在问题：
		目前400版本的model有问题，无法创建、需要优化
		具体350或是400，主要根据需求来的，【t_cfg_cserverinfo插入时会指定是什么版本的】、有对应的版本模拟器


C模拟器项目中的lsc400_data.xlsx文件[只需修改下面两个sheet，其余不用  -- 注意v350和v400的文件有点不一样]
	m_site  sheet页
...
```
以上海为例
	首先是在空间里，创建新站点【站点下做任何操作、通过同步来实现】
		数据中心还要创建楼栋
...
以上海【接入广东】为例
	provice_name			为步骤4、C接口启动读取的yml配置里的province_name【用于区分是哪个省份接入的】						  即t_cfg_cserverinfo的lsc_name
	...
	注意：集团的interface_type ，如果是c接口接入必须写1
...
集团
	...
	01-08-08-04-03-02	贵州贵数据中心_液冷2
	01-08-08-04-03-01	贵州贵数据中心_液冷1
	01-08-08-04-02		贵州贵通信枢纽楼_液冷
...
以上海为例
	...
	方法1【按站点稽核同步 -- 目前用的这个】：
		...
		接口位置：
			/v1/cinterface/compareAndSyncBySite（get请求）   -- 出现这个问题就是config有问题，
		...
		例子
			...
			[集团]
			服务：
				test-spider-cinterfacetcpclient
				test-spider-cinterfacereader  -- 同步历史告警
        	curl --location 'http://10.1.202.2:31147/v1/cinterface/compareAndSyncBySite?operateUser=chw-0919&siteId=01-08-08-04-03-02&justCompare=false'
        	curl --location 'http://10.1.4.243:8097/v1/cinterface/compareAndSyncBySite?operateUser=chw-0919&siteId=01-08-08-04-03-02&justCompare=false'
