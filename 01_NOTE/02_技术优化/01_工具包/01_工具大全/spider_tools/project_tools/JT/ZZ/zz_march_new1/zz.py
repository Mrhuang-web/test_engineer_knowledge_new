...
# 在 zzallinonewriter.py 末尾追加即可（原代码不动）
# ================= 工厂类（可选） ================= #
class WriterFactory:
    """
    完全兼容 common_zz_match.py 的调用习惯：
    writer = WriterFactory.create_writer('air', batch_num='20250729', ...)
    writer.write(count=100)
    """

    @staticmethod
    def create_writer(index_category: str, *, es_url: str = ESConfig.ES_URL, **kwargs) -> 'DeviceWriter | MetaWriter | MapWriter | LinkWriter':
        """
        外部唯一入口：字符串 -> 写入器实例
        """
        # 统一把 es 实例和索引名都准备好
...
# ================= 复用你原来的 ZZMatchSpider ================= #
class ZZMatchSpider:
    """仅把内部『WriterFactory.create_writer』换成我们新的工厂即可，其余一行不动"""
    ...
    # 以下三个主流程函数 **完全不用改**，因为内部只调了 WriterFactory.create_writer
    ...
    # 以下三个内部函数也 **无需改动**，因为只调了 WriterFactory.create_writer
    ...
    def _create_room_and_property_and_irm(self, room_name: str, room_id: str):
        ...
        prop_writer = WriterFactory.create_writer(
            ...
            province_id=self.province, city_id=self.city, site_int_id=self.zh_label,
            irms_province_code=self.irms_province_code, cutin_date=self.cutin_date,
            power_room_type=self.power_room_type, power_supply_mode=self.power_supply_mode,
            county_id=self.county_id
        )
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="ZZ 22 类索引一次性写入 ES")
    parser.add_argument("--es", default="http://10.1.203.38:9200", help="ES 地址")
    parser.add_argument("--batch", default="20250729", help="batch_num 前缀（决定索引后缀）")
    parser.add_argument("--count", type=int, default=10, help="每索引写多少条")
    parser.add_argument("--indices", nargs='+', help="只写指定索引，空格分隔；不写则全部")
    args = parser.parse_args()

    writer = ZZAllInOneWriter(es_url=args.es, batch_num=args.batch)
    if args.indices:
        writer.write_indices(args.indices, count=args.count)
    else:
        writer.write_all(count=args.count)
