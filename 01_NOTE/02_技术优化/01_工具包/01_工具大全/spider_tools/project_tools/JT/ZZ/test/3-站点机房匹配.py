...
class DataBuilder:
    ...
    def build_device_docs(device_type: str, count: int, context: Dict) -> List[Dict]:
        ...
        if not fields:
            ...
            print(f"  ⚠️  设备类型 '{device_type}' 未预定义，使用通用字段")
    ...
    def _generate_field_value(field: str, context: Dict, device_type: str = None) -> Any:
        ...
        if field == 'province_id':
            return context.get('province_id', '520000')
        elif field == 'city_id':
            return context.get('city_id', '520400')
        elif field == 'county_id':
            return context.get('county_id', '000000')
        elif field == 'batch_num':
            return context.get('batch_num', f"BATCH_{datetime.now().strftime('%Y%m%d')}")
        elif field == 'collect_time':
            return context.get('collect_time', datetime.now().strftime('%Y%m%d%H%M%S'))
        elif field == 'zh_label':
            return context.get('zh_label', f"{device_type}_{random.randint(1000, 9999)}")
        elif field == 'site_int_id':
            return context.get('site_int_id')

        # 设备特有字段
        if field == 'estimated_retirement_time':
            return (datetime.now() + timedelta(days=random.randint(1000, 5000))).strftime('%Y-%m-%d')
        elif field == 'start_time':
            return (datetime.now() - timedelta(days=random.randint(100, 1000))).strftime('%Y-%m-%d')
