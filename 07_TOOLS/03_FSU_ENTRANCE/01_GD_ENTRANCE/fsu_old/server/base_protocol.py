    ...
    def _generate_response(self, protocol_config: Dict[str, Any], parsed_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(frame_rule, dict):
                ...
                # 2. 检查是否为事件类型，只有事件类型才动态评估
                # 从协议配置中获取事件类型列表，默认为空列表
                event_types = protocol_config.get("protocol", {}).get("event_types", [])
                # 检查当前数据帧类型是否在事件类型列表中
                is_event_type = actual_type in event_types
                
                # 3. 检查协议配置中的动态时间开关
                dynamic_time_enabled = protocol_config.get("protocol", {}).get("dynamic_time_enabled", False)
                
                # 4. 综合判断是否需要动态评估
                should_evaluate_time = is_event_type and dynamic_time_enabled and not performance_mode
