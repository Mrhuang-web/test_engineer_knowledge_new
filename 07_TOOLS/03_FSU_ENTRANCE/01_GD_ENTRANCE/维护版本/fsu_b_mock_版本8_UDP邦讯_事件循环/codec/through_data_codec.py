...
class ThroughDataCodec:
    ...
    def encode(self, parsed_data: Dict[str, Any], response_data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            # 优先从response_data中获取data_frame_type
            data_frame_type = response_data.get("data_frame_type", "")
            self.logger.debug(f"从response_data获取的data_frame_type: '{data_frame_type}'")
            if not data_frame_type:
                # 从parsed_data中获取
                data_frame_type = self._get_data_frame_type_from_parsed(parsed_data)
                self.logger.debug(f"从parsed_data获取的data_frame_type: '{data_frame_type}'")
            self.logger.debug(f"最终使用的数据帧类型: '{data_frame_type}'")
            ...
            # 特殊处理108D指令的时间格式（使用压缩时间格式）
            self.logger.debug(f"检查数据帧类型: {data_frame_type}")
            if data_frame_type == "108D":
                self.logger.debug("特殊处理108D指令：使用压缩时间格式")
                # 生成当前时间
                import time
                now = time.localtime()
                year = now.tm_year - 2000  # 相对2000年
                month = now.tm_mon
                day = now.tm_mday
                hour = now.tm_hour
                minute = now.tm_min
                ...
                self.logger.debug(f"当前时间: {now.tm_year}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:{second:02d}")
                self.logger.debug(f"压缩时间格式输入: year={year}, month={month}, day={day}, hour={hour}, minute={minute}, second={second}")
                ...
                self.logger.debug(f"编码后的值: ymd=0x{ymd:04X}, hms=0x{hms:04X}")
                
                # 转换为4字节，按照协议文档的顺序：[ymd_low, ymd_high, hms_low, hms_high]
                # 年月日：低位字节在前
                ymd_low = ymd & 0xFF
                ymd_high = (ymd >> 8) & 0xFF
                # 时分秒：低位字节在前
                ...
                # 构建时间戳字节：按照协议文档示例顺序
                # 示例：B3 02 0D 5D 对应 02B3[年月日] 5D0D[时分秒]
                time_bytes = bytes([ymd_low, ymd_high, hms_low, hms_high])
                self.logger.debug(f"压缩时间戳字节: {time_bytes.hex().upper()}")
                
                # 构建新的data_frame_data，替换时间字段
                # 108D响应结构：data_frame_type(2) + card_id(2) + vendor_id(1) + status(1) + 时间戳(4) + 填充
                # 所以时间戳应该放在第2+2+1+1=6字节位置，长度4字节
                self.logger.debug(f"原始data_frame_data长度: {len(data_frame_data)}")
                self.logger.debug(f"原始data_frame_data: {data_frame_data.hex().upper()}")
                
                # 提取前6字节：data_frame_type(2) + card_id(2) + vendor_id(1) + status(1)
                if len(data_frame_data) >= 6:
                    prefix = data_frame_data[:6]
                    self.logger.debug(f"前缀数据: {prefix.hex().upper()}")
                    
                    # 构建新的数据：前缀 + 时间戳(4) + 填充
                    # 总长度应该与配置文件中的data_frame_length一致
                    new_data = prefix + time_bytes
                    
                    data_frame_data = new_data
                    self.logger.debug(f"替换时间字段后的数据: {data_frame_data.hex().upper()}")
                    self.logger.debug(f"替换后长度: {len(data_frame_data)}")
