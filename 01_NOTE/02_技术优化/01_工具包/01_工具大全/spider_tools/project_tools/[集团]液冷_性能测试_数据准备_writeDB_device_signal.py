...
# 一、根据站点、设备、信号，写活动告警和消除告警数据 way控制是要生成历史还是实时告警
def create_activealarm(number=1000, filename='d_activealarm_by_device_signal.sql', siteId="", signalid="",deviceid="",way="",alert_time_begin="",alert_time_end=""):
...
def create_tah_by_device_signal(number=100, filename='d_tah_by_device_signal.sql', siteId="", begin_time="", ent_time="",
                                deviceid='', signalid=""):
    # siteId=620402005
    ''' '''
    global d_tah_id_SN
    ...
    # 2. 根据siteId和deviceid和signalid查询信号数据
    ...
    print('信号历史数据：查询结果为：', count[0][0], '个')
