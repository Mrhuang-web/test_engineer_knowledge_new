...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('02_kafka_monitor_entrance_encode_command.log'),
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    self.consumer = KafkaConsumer(
                        ...
                        # auto_offset_reset 可选值：
                        #   'earliest': 当没有偏移量可提交或偏移量无效时，从最早的消息开始消费
                        #   'latest': 当没有偏移量可提交或偏移量无效时，从最新的消息开始消费
                        #   'none': 当没有偏移量可提交或偏移量无效时，抛出异常
...
def main():
    ...
    """
    # 配置参数
    bootstrap_servers = '10.1.203.121:9095'

    # 提示用户输入要监听的 topic
    # topic = input('请输入要监听的 topic 名称（例如：test_topic）: ')
    topic = 'entrance_encode_command'
    if not topic:
