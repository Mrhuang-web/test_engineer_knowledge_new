...
#一、根据站点、写活动告警和消除告警数据 way控制是要生成历史还是实时告警
def create_activealarm(number=1000, filename='d_activealarm_bysiteID.sql', siteId="2012",way="",alert_time_begin="",alert_time_end=""):
    '''一次856条'''
...
# 二、根据站点ID写全量历史数据
def create_tah_by_siteID(number=100, filename='d_tah_by_siteID.sql', siteId="", begin_time="", ent_time=""):
    ...
    print(sql)
    #统计多少个结果
    sql2= f'''SELECT  count(*) FROM m_signal WHERE  SiteID IN ({siteId}) AND `Type`!=4'''
    print(sql2)
    result = get_sql_result(sql)
    result2 = get_sql_result(sql2)
    #print('m_signal查询结果:', result)
    print('生成历史数据：查询结果为：',result2[0][0],'个')
    ...
    # 第二种按天数来循环给值  要换就换下面的循环days 暂没用这个
    delta_days = (time2 - time1).total_seconds() / 86400
    print('生成历史数据：时间相差：', delta_days, '天')
    ...
    for li in result:
        if lines >= number:
            break
        for hour in range(int(delta_hours) + 1):
            current_time = time1 + timedelta(hours=hour)
...
def get_sql_result(sql='select a from b'):
    ...
    # 集团山西
    conn = pymysql.connect(host='10.1.203.38', port=3306, user="root", password="G$SGp!8L3O",  database="cinterdb_400_jt_gz", charset='utf8mb4')
...
if __name__ == "__main__":
    ...
    create_activealarm(
        ...
        alert_time_end="2025-09-12 03:00:00",
    )


    create_tah_by_siteID(
        number=1000,
        siteId="2011,2012",
        begin_time="2025-09-09 00:00:00",
        ent_time="2025-09-12 03:00:00"
    )
