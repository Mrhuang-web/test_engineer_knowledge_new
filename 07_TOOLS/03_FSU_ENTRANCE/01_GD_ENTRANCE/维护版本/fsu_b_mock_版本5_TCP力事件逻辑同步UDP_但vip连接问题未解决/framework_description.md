...
1. 迁移了enabled字段 ：
   ...
   - 从 events.json 文件中移除了 enabled 字段
   - 在 protocol_configs.json 文件中，为每个协议配置添加了 enabled 字段
2. 修改了event_center.py文件 ：
   
   - 移除了全局的 event_enabled 字段
   - 移除了相关的 set_event_enabled 和 get_event_enabled 方法
   - 修改了 check_and_update_response 方法，让它检查每个协议配置的 enabled 字段
   - 修改了 init_events 方法，移除了从 events.json 文件中读取 enabled 字段的代码
3. 测试了修改后的代码 ：
   
   - 运行event_center.py文件，测试配置加载功能
   - 测试结果显示配置加载成功，包括5个事件和3个协议配置
现在， enabled 字段的作用是控制每个协议配置是否启用，而不是控制整个事件中心的开关。这样，您可以为每个协议配置单独设置是否启用，更加灵活。

例如，如果您想启用 liwei_udp 协议的事件处理，只需要将 protocol_configs.json 文件中 liwei_udp 配置的 enabled 字段设置为 true 即可，而不会影响其他协议的配置。