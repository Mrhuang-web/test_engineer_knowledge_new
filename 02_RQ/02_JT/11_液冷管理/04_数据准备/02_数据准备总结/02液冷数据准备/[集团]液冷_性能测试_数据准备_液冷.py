...
def create_tah_by_device_signal(f=None, number=100, filename='d_tah_by_device_signal.sql',
    ...
    if mode == "0":
        ...
        write_signal(f, d_tah_id_SN, signal_result, filename, begin_time, end_time, number, time_list)
        return
    ...
    if mode == "1":
        if device_no == "1":
            ...
            signal_ids_str = ",".join([f"'{id[0]}'" for id in signal_rows])
    ...
    for x in range(2025, 2028):  # 站点控制
        for y in range(1, 14):  # 机房控制
            ...
            create_tah_by_device_signal(
                ...
                begin_time="2025-10-26 00:00:00",
