...
# 一、根据站点、设备类型，写活动告警和消除告警数据 way控制是要生成历史还是实时告警
def create_activealarm(number=1000, filename='d_activealarm_by_deviceType2.sql', siteId="", devicetype="",way="",alert_time_begin="",alert_time_end=""):
    ...
    #sql = 'SELECT  SCID,  SiteID  ,DeviceID ,`Type`, SignalID,SignalNumber,SignalName,AlarmLevel FROM m_signal  ggg  WHERE  SiteID IN (' + siteId + ') AND  ggg.`Type` in (0,1,2,3,4) AND SignalID IN (' + devicetype + ')'
    sql = f'''SELECT SCID,  SiteID  ,DeviceID ,`Type`, SignalID,SignalNumber,SignalName,AlarmLevel 
                     FROM m_signal 
                     WHERE SiteID IN ({siteId})
                     AND `Type` =0
                     AND SignalID REGEXP '^({devicetype})' '''  # 正则写法，匹配多个类型
    # 匹配单个的情景：  AND SignalID like '%{devicetype}%'
    ...
    while lines < number:
        ...
        for li in result:
            ...
            line = template_line % (
                d_activealarm_id_SN, SCID, SiteID, DeviceID, Type, SignalID, SignalNumber, SignalName, SignalID,
                serial_no,
...
# 二、根据站点ID、设备类型写全量历史数据
def create_tah_by_deviceType(number=100, filename='d_tah_by_deviceType2.sql', siteId="", begin_time="", ent_time="",devicetype=""):
    ...
    sql3 = f'''SELECT count(*) FROM m_signal WHERE SiteID IN ({siteId}) 
                 AND `Type` != 4 
                 AND SignalID REGEXP '^({devicetype})' '''
    #print(sql3)
    count = get_sql_result(sql3)
    print('生成历史数据：信号条数查询结果为：', count[0][0], '个')

    signal_result = get_sql_result(sql)
    #print(sql)
    if not signal_result:
        print('生成历史数据：m_signal表，没找到输入站点或对应类型前缀（3位）',devicetype,'的测点信息')
        return
...
if __name__ == "__main__":
    ...
    create_activealarm(
        ...
        alert_time_end="2025-09-11 00:00:00"
    )

    create_tah_by_deviceType(
        # filename='',
        #类型用“|” ，分隔
        devicetype='016|015',
        number=100,
