...
class esDB():
    ...
    def insert_esdata_device_alert(self, flush=True):
        '''
            生成特定区域的某个测点的数据
            :return:
        '''
        ...
        es = Elasticsearch(self.url)
        # todo
        filename = "alert_" + creattime + "_" + self.precinct_id[0:5]  # 不用上海，就需要改成这个
        # filename = "fsu_" + "point_data_" + creattime   # 上海断电分析专用索引
        indetype = "point_history_data"
        urlputindex = self.url + "/" + filename
        urlputmapping = self.url + "/" + filename + "/" + indetype + "/_mapping"
        requests.put(urlputindex)  # 新增index
    ...
    def insert_esdata_device(self, flush=True):
        ...
        es = Elasticsearch(self.url)
        # todo
        filename = "fsu_" + creattime + "_" + self.precinct_id[0:5] # 不用上海，就需要改成这个
        # filename = "fsu_" + "point_data_" + creattime   # 上海断电分析专用索引
        indetype = "point_history_data"
        urlputindex = self.url + "/" + filename
        urlputmapping = self.url + "/" + filename + "/" + indetype + "/_mapping"
        ...
        # param={"point_history_data":{"properties":{"channelID":{"type":"keyword"},"collectTime":{"type":"date"},"desc":{"type":"keyword"},"deviceID":{"type":"keyword"},"deviceName":{"type":"keyword"},"indexSeq":{"type":"date"},"manufacturerId":{"type":"keyword"},"measureVal":{"type":"float"},"meteID":{"type":"keyword"},"precinctID":{"type":"keyword"},"precinctName":{"type":"keyword"},"signalNumber":{"type":"keyword"},"type":{"type":"keyword"}}}}
        pload = json.dumps(self.esparam)
        urlputmapping_result = requests.put(urlputmapping, pload)  # 创建mapping
        ...
        for i in r:
            ...
            actions.append(action)
            if len(actions) == 100:
                results = helpers.bulk(es, actions)
                del actions[0:len(actions)]
        helpers.bulk(es, actions)
        if flush:
            es.indices.refresh(index=filename)
...
if __name__ == '__main__':
    ...
    #插入数据
    list_m=['008349']
    #list_m=['017301','017302']
    list_d=['2021-04-21']
    v=100
    for d in list_d:
        for m in list_m:
            #向节点下指定设备指定测点写入数据
            esDB(precinct_id="01-25-01-08-01",mete_code=m,imdate=d,minval=10,maxval=100,device_id='03113006000003306064',estime='T11:30:23',env="release").insert_esdata_device()
            #向节点下所有设备下指定测点写入数据
            esDB(precinct_id="01-25-01-08-01",mete_code=m,imdate=d,minval=10,maxval=100,estime='T11:30:23',env="release").insert_esdata_province()
        v=v+100
    esDB(precinct_id="01-02-10-04-35-01", mete_code="006309", imdate="2023-09-17", minval=120, maxval=120 ,device_id='16881181054244492',  env="release").insert_esdata_device_batch()

    start_time = time.perf_counter()
    for i in range(100): #100  17s
        esDB(precinct_id="01-02-10-04-35-01", mete_code="006309", imdate="2023-09-17", minval=120, maxval=120 ,device_id='16881181054244492',  env="release").insert_esdata_device_batch()
    end_time = time.perf_counter()
    execution_time = end_time-start_time
    print("代码执行时间：", execution_time, "秒")

   #云南能耗有小时级别的数据，可用如下方式造数据
    esDB(precinct_id='01-32-13-10-09', imdate="2024-02-18", del_col='meteID', del_clo_v="092316").del_esdata_query()
    esDB(precinct_id='01-32-13-10-09', imdate="2024-02-19", del_col='meteID', del_clo_v="092316").del_esdata_query()
    esDB(precinct_id='01-32-13-10-09', imdate="2024-02-20", del_col='meteID', del_clo_v="092316").del_esdata_query()
    esDB(precinct_id='01-32-13-10-09', imdate="2024-02-21", del_col='meteID', del_clo_v="092316").del_esdata_query()
    esDB(precinct_id="01-32-13-10-09", mete_code="092316", imdate="2024-02-18", minval=120, maxval=120, device_id='00531006000005864974', env="release").insert_esdata_device_batch()
    esDB(precinct_id="01-32-13-10-09", mete_code="092316", imdate="2024-02-19", minval=240, maxval=240, device_id='00531006000005864974', env="release").insert_esdata_device_batch()
    esDB(precinct_id="01-32-13-10-09", mete_code="092316", imdate="2024-02-20", minval=360, maxval=360, device_id='00531006000005864974', env="release").insert_esdata_device_batch()
    esDB(precinct_id="01-32-13-10-09", mete_code="092316", imdate="2024-02-21", minval=480, maxval=480, device_id='00531006000005864974', env="release").insert_esdata_device_batch()
"""
