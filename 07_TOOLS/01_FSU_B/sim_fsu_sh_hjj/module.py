...
# todo 使用的是sqlalchemy语法， query里面传表名（表名就是上面定义的类）
def query_signals_by_device_type(fsuid, deviceid,all_record=True):
    result = None
    record = Session().query(Device).filter_by(fsuid=fsuid).filter_by(deviceid=deviceid).order_by('devicetype') \
        .first()
    # if all_record == True:
    #     record = Session().query( Signals ).filter_by(fsuid=fsuid).filter_by( deviceid = deviceid ).order_by('signalsid')\
    #         .all()
