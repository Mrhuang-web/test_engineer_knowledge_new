...
class DataBuilder:
    DEVICE_FIELDS = {
        'site': [
            'china_tower_station_code', 'village_pass_serv_code', 'cutin_date', 'site_site_type', 'latitude',
            'floor_number',
        ...
        'room': [
            ...
            'room_room_area', 'row_direction', 'row_num', 'shared_unit', 'start_column', 'start_row',
            'tele_cmn_serv_pro_code', 'tele_cmn_serv_pro_name', 'uuid', 'village_pass_serv_code',
    ...
    def _generate_field_value(field: str, context: Dict, device_type: str = None) -> Any:
        ...
        if field == 'res_code':
            if device_type == 'room':
                return context.get('room_res_code', f"room_{random.randint(1000, 9999)}")
            if device_type == 'site':
                return context.get('site_res_code', f"site_{random.randint(1000, 9999)}")
            if device_type == 'site_property':
                return context.get('site_property_res_code', f"site_{random.randint(1000, 9999)}")
            if device_type == 'room_property':
                return context.get('room_property_res_code', f"site_{random.randint(1000, 9999)}")
            if device_type not in ('room', 'site', 'site_property', 'room_property'):
                return context.get('res_code', f"device_{random.randint(1000, 9999)}")
        if field == 'zh_label':
            if device_type == 'room':
                return context.get('room_zh_label', f"room_{random.randint(1000, 9999)}")
            if device_type == 'site':
                return context.get('site_zh_label', f"site_{random.randint(1000, 9999)}")
            if device_type == 'site_property':
                return context.get('site_pro_zh_label', f"site_{random.randint(1000, 9999)}")
            if device_type == 'room_property':
                return context.get('room_pro_zh_label', f"site_{random.randint(1000, 9999)}")
            if device_type not in ('room', 'site', 'site_property', 'room_property'):
                return context.get('device_zh_label', f"device_{random.randint(1000, 9999)}")
        ...
        if field == 'dh_id':
            return context.get('room_dh_id', '')
        if field == 'dh_name':
            return context.get('room_dh_name', '')
        if field == 'zg_id':
            return context.get('room_zg_id', '')
        if field == 'zg_name':
            return context.get('room_zg_name', '')
        ...
        if field == 'retire_time':
            return context.get('retire_time', datetime.now().strftime('%Y-%m-%d'))
        if field == 'start_time':
        ...
        if field == 'device_type':
            return device_type
        if field == 'property_unit':
            return context.get('property_unit', '中国移动')
        ...
        if field == 'device_number':
            return str(random.randint(1, 10))
        if field == 'space_room_type':
            return context.get('space_room_type', '动力机房')
        if field == 'vendor_id':
            return 'test'
        ...
        if field == 'qr_code_no':
            return ''
        if field == 'equiproom_type':
            return context.get('equiproom_type', '传输机房')
