...
import subprocess
import threading
from configs.url_config import url_mapping, server_config, hot_restart_config, external_config
...
class MockServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _check_security(self, path, params):
        """检查安全校验，针对动环external接口"""
        # 动环external接口前缀
        external_prefix = '/v1/external/'
        
        # 只有动环external接口需要安全校验
        if not path.startswith(external_prefix):
            return True, None
        
        # 如果未启用认证，直接通过
        if not external_config['enable_auth']:
            return True, None
        
        # 检查请求中是否包含密钥
        if 'secret_key' not in params:
            return False, {'code': '22', 'desc': '安全校验不通过', 'data': ''}
        
        # 验证密钥是否正确
        if params['secret_key'] != external_config['secret_key']:
            return False, {'code': '22', 'desc': '安全校验不通过', 'data': ''}
    ...
    def _handle_request(self):
        ...
        try:
            ...
            # 检查安全校验（针对动环external接口）
            is_secure, security_error = self._check_security(path, params)
            if not is_secure:
                logger.warning(f"Security check failed for {path}: {params} - Response: {security_error}")
                self._get_response(security_error)
                return
            ...
            if service_name in self.services:
                ...
                if hasattr(service, method_name):
                    ...
                    # 检查是否是动环external接口，如果是，确保响应格式正确
                    if path.startswith('/v1/external/'):
                        # 动环接口需要使用code=00表示成功
