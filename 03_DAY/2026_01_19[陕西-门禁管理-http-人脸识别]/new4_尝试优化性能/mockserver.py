...
class MockServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _parse_params(self, method):
        ...
        # 先处理GET请求的URL查询字符串，避免不必要的body读取
        if method == 'GET':
            if '?' in self.path:
        ...
        if content_length > 0:
            ...
            # 尝试解析为JSON格式（优先级最高）
            try:
                json_params = json.loads(body.decode('utf-8'))
                params = json_params
                return params
            except json.JSONDecodeError:
                # 如果JSON解析失败，且不是JSON Content-Type，尝试解析为表单数据
                if 'application/json' not in content_type:
                    try:
                        form_params = dict(urllib.parse.parse_qsl(body.decode('utf-8')))
                        params = form_params
                    except Exception:
                        pass
    ...
    def _get_response(self, response_data):
        """生成HTTP响应（优化版，减少中间字符串创建）"""
        self.send_response(200)
        self.send_header('Content-Type', 'application/json')
        self.end_headers()
        # 直接生成JSON字节，减少中间字符串
        self.wfile.write(json.dumps(response_data, separators=(',', ':'), ensure_ascii=False).encode('utf-8'))
    
    def _handle_request(self):
        """处理请求（优化版，移除所有日志记录，减少I/O开销）"""
        method = self.command
...
def run_server():
    ...
    with socketserver.ThreadingTCPServer((host, port), MockServerHandler) as httpd:
