...
"""
excel_file = './lsc400_data_jtyl.xlsx'
...
d_device_id =0 # 40002 SELECT   *   FROM    m_device   WHERE DeviceID>42405
signals=[]
def load_xls_area():
    wb = load_workbook('./lsc400_data_jtyl.xlsx')
    ws=wb['m_area']
    result_list=[]
    ...
    for area in area_list:
        ...
        for room in room_filter_list:
            ...
            for device in device_filter_list:
                ...
                # todo 控制写入的自己想要的设备名
                # # 下方判断为if m_device == signal_m,只写入了通用的了
                # signal_m = signal_list[0][0]
                # 下方判断为if m_device in signal_m,写入了指定的
                signal_m = ["液冷-工况环境","液冷-系统参数","液冷-一次侧机组（冷机/冷塔）","液冷-CDU"]


                # 后续可定制某设备特定测点组合(通过修改条件  in时改成指定的)
                # 注意，上面只是判断了写入哪类设备，但是写入的测点还是没有分开的，就是对指定设备类型插入了全部的数据了(有一个是问题就是还有其他数据？)
                # if signal[0] == m_device 加上这个后，那么插入只会对应测点到对应设备
                if m_device in signal_m:
                    logger.debug('["开始写入测点编码m_device,signal_m %s %s" \n\t', m_device, signal_m)
                    global signals
                    signals=[]
                    pass
                    for signal in signal_filter_list :
                        if signal[0] == m_device:
                            signal_SCID = dSCID
