...
JOIN t_cfg_metemodel_detail t2 ON t1.device_model=t2.model_id AND t1.device_type IN (6,8,87,92,2,11,12,15,17) AND t1.rated_power >'0' AND t1.precinct_id LIKE  :precinct_id   AND t2.mete_code IN ('006309','008318','008319','008320','008338','','008339','008340','008349','008350','008351','011301','012301','015201','015203','015303','015403','017301','002307','002314','002321','087309','092308','092309','092310','092330')
 ...
 LEFT JOIN t_cfg_dict m ON m.dict_code = t1.device_type AND m.col_name = 'device_type'
...
LEFT JOIN t_cfg_precinct AS building ON  building.precinct_id  !=(SELECT  tcs.site_id FROM t_cfg_site tcs WHERE tcs.site_id=u.precinct_id)  AND building.precinct_id=p.up_precinct_id   
WHERE p.isdel= 0  
AND t1.device_id = :device_id   LIMIT 1  ;
SELECT  t2.mete_code AS meteCode,
	p.precinct_name AS precinctName,
	t1.rated_power,
	t1.precinct_id AS precinctId,
 ...
 WHERE p.isdel= 0 ;
SELECT
    room.precinct_id AS roomid,
    room.precinct_name AS roomname,
    room.room_kind AS roomkind,
    building.precinct_id AS buildingid,
    building.precinct_name AS buildingname,
    dc_site.precinct_id AS dcsite_id,
    dc_site.precinct_name AS dcsite_name,
    site.precinct_id AS siteid,
    site.precinct_name AS sitename,
    site_s.site_type AS sitetype,
    county.precinct_id AS countyid,
    county.precinct_name AS countyname,
    city.precinct_id AS cityid,
    city.precinct_name AS cityname
FROM t_cfg_precinct room
	LEFT JOIN t_cfg_precinct building ON room.up_precinct_id = building.precinct_id AND building.precinct_kind = 3
	LEFT JOIN t_cfg_precinct dc_site ON building.up_precinct_id = dc_site.precinct_id AND dc_site.precinct_kind = 2
	LEFT JOIN t_cfg_precinct site ON room.up_precinct_id = site.precinct_id AND site.precinct_kind = 2
	LEFT JOIN t_cfg_site site_s ON site_s.site_id = COALESCE(site.precinct_id, dc_site.precinct_id)
	LEFT JOIN t_cfg_precinct county ON left(site_s.site_id,11) = county.precinct_id
	LEFT JOIN t_cfg_precinct city ON county.up_precinct_id = city.precinct_id
	WHERE room.room_kind IS NOT NULL AND city.precinct_id LIKE :precinct_id;