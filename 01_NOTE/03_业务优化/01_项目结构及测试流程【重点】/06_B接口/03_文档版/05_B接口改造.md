...
当前B接口启动一个节点，所有工作由B接口一个程序完成，容易出现性能瓶颈。
随着FSU接入数量逐渐增加，B接口作为单节点，出现故障的频率越来越高。

### 2、改造点

- B接口拆分成 client, server, dataHandle 三种启动模式

  - client端的职责：接收上层的请求（比如currentmonitor调用的获取实时数据，下发指令），并完成向FSU的请求。
  - server端的职责：接收FSU上报的数据，比如注册数据，配置数据，告警数据等
  - dataHandle端的职责：完成数据持久化。比如：从client端收到FSU报上来的实时数据后，client端把数据转发到kafka, 再由dataHandle端监听并消费kafka的数据，并完成入库（写redis, ES）

### 3、各模式下的改造点

改造项 修改项
client 实时数据获取 1）请求合并：合并n秒内的下发指令（接口请求和kafka都进行合并）

kafka方式请求：合并n秒内的下发指令，并将响应结果打入kafka（消费者：dataHandle），
client端并不直接处理数据。 1、调用“v1/currentmonitor/getMeasureVal”接口，查看指令合并情况
2、N秒是可配置项，对应的配置项是：request.merge.interval:5

3、合并就是多交请求共用一次的结果（5秒内请求过，不请求，返回的数据在5秒内，不请求）

定时任务-fsu状态检测 B接口内部的定时任务之一：FSU状态检查，即每5分钟向所有的FSU发起一次状态检查。

之前的做法就是把所有的FSU加载到内存，然后按150个FSU一个包，放到KAFKA里面。
再由消费者完成KAFKA状态检测工作。

修改之后：
1）改为一个FSU一条消息推到kafka
2）如果当前有正在处理中的FSU，则不重复推到kafka里面，避免无用的状态检测请求。 1）定时任务包的大小配置：fsu_total_per_msgpack:1 （ms-binterface-prod.yml 中进行配置）

2）添加判断当前是否有正在检测中的FSU，如果有，则不重复添加。

3）其它逻辑不变。
server FSU数据上报 1）之前只有一个B接口节点接收超过4000个FSU上报的数据，节点性能出现瓶颈，
2）现使用2个或以上的节点进行接收FSU数据上报
3）需要接入Nginx进行代理
4）FSU配置上报的地址为 Nginx监听的地址。 1、在ng监听28080端口，FSU把数据上报，先通过NG的 28080端口，然后再由NG路由颁发给两个server 模式启动的B接口

2、通过上报注册数据 / 配置数据，可看到路由到2个 server模式的B接口
dataHandle 定时任务-历史数据同步等任务 定时任务分发改造为1个fsu对应一个 B接口的任务包大小都是同一个配置项。
1）定时任务包的大小配置：fsu_total_per_msgpack:1 （ms-binterface-prod.yml 中进行配置）

kafka消费实时数据响应结果 新增，消费持久化 1）获取设备实时数据时，CLIENT端和FSU进行交互，FSU把设备测点数据响应给CLIENT端
...
```
docker save -o /home/sudoroot/new_binterface.tar cd0d7839534dscp -r /home/sudoroot/new_binterface.tar sudoroot@10.12.12.184:/home/sudoroot/new_binterfacedocker load -i /home/sudoroot/new_binterface/new_binterface.tardocker tag cd0d7839534d binterface-service
```

1、B接口启动脚本 (CLIENT模式）

```
   docker run --name binterface-service-client-p1 --net host  --log-driver=json-file --log-opt max-size=30m --log-opt max-file=3 --env SERVER_PORT=18181 --env EN    V_EUREKA=http://10.12.12.184:8761/eureka/ --env spring.cloud.inetutils.preferred-networks=10.12.12.186 --env ENV_CONFIG_IP=10.12.12.184 --env ENV_CONFIG_PORT    =18888 --env ENV_TYPE=prod --env RUN_MODE=client  -v /tmp/logs/binterface-service-new:/opt/data/logs/ -d binterface-service:latest
```

2、B接口启动脚本 (SERVER模式 或 DATA）
...
1、只起client节点，应该是只能向服务端发起请求，不处理数据。实际发送告警会直接被处理,注册上报接口成功，配置注册失败
2、检查datahandle,综合视图实时数据采集情况，有没有数据，数据采集时间是否是最新的
3、考虑b接口被拆分3种服务模式来处理b接口数据（
1）假设client-p1,server-p1,datahandle-p1,三个模式在一台机器启动，client-p2,server-p2,datahandle-p2在另外一台机器启动
2）假设client-p1，client-p2在A机器上启动，server-p1，server-p2在B机器上启动，datahandle-p1,datahandle-p2在C机器上启动
3）假设client,server,datahandle，6个服务节点都在同一台机器上启动
4）6个服务节点同台机器启动处理数据速度，与原旧逻辑B接口启动，性能对比
5）同样并发情况下，服务被分发到不同机器节点处理数据，性能一定好吗）
4、检查性能波动值，找到最大吞吐量
5、CPU消耗在那个阶段开始飙高
6、同种启动模式，p1,和p2会不会做一个的事情？，比如P1、P2都处理同一件事件 。目前通过日志查看处理数据不同，也就是事件处理不会重复操作。但是有时监控日志会报错，如下Caused by: java.net.SocketTimeoutException: Connect timed out
at java.base/sun.nio.ch.NioSocketImpl.timedFinishConnect(NioSocketImpl.java:546)
7、考虑并发情况，目前一个FSU设备测点1260个device code，当前测试机房500个平均每个机房都接入一个fsu,假如并发告警，1260 ✖️ 500=630000个，如果后续增量压测5000个fsu,也就是1260 ✖️ ****5000=63000000个。如果一个机房接入多个FSU，一个注册上来，第一个检查是设备会不会重复（预期是：不会重复，实际同一个机房重新接入FSU，会按设备处理不会重复，第二个就是设备ID会不会变）

### 6、新建500个机房、每个机房接入1个FSU
...
```
cd /opt/apache-jmeter5.5/bin/[root@dh186 ~]# ./jmeter-server -Djava.rmi.server.hostname=10.12.12.186./jmeter-server -Djava.rmi.server.hostname=10.12.70.59
```

5、控制点需要配置工作节才能监听和下发
工作节点server感觉不稳定，不知道是不是jdk版本不一致的问题，会出现连接超时或者服务自动停止2024-02-05 18:04:34,097 ERROR o.a.j.e.DistributedRunner: The following remote engines have not started:[10.12.12.186:1099]
...
8、分布式压测不适用fsu上送数据,原因：控制🐔分发slave节点做一样的事,而b接口是实现高效消费，不做重复事件，因此不适用分布式。比如控制点+2slave工作节点，100个线程，分发出去也就是300个线程，200个线程是做一样的事情。适合单机击压测，但是jmeter 单机压测能力是上限的，并不能很好到的压测出性能的B接口瓶颈。从某种角度看，对于b接口业务上逻辑重复数据是无意义的，但是也会给服务器产生压力类似攻击。
9、B接口改造之前超过4000个FSU上报的数据，节点性能出现瓶颈，因此本次改造压测需要在此之上，然后不断增量式压测来测试B接口性能瓶颈。