...
业务说明：
	...
	4、t_zz_power_specialty 中 某个为0才是动环属性里面有的
...
3、其他地方展示的是动环
	...
	SELECT * from t_cfg_dict WHERE col
...
需要重新梳理：
	疑问：为什么zz的没有自动入库进去
	1、SELECT * from  zz_power_dict_mapping;   （全部数据共用这份配置表）
		这个表就是提供用来映射，还有查询时匹配？？
	2、找到动环的数据
	3、找到综资的数据
	4、综资映射到动环？？
...
产品对细节：
	运行分析-市电停电配置：必须是展示动环的（需要参与计算）
	监控必须是综资（监控这块都是综资的）
	其他地方也统一变为动环
	容量管理-容量概览-机楼
		通过点击地图，旁边有楼栋等信息弹窗，再点击就能跳转过去
