...
"""
钛迪ES2200门禁控制器协议解析脚本
用于解析钛迪ES2200门禁控制器协议的16进制码流
"""

from base_parser import BaseParser

class TidiES2200Parser(BaseParser):
    """钛迪ES2200门禁控制器协议解析器"""
    
    def __init__(self):
        super().__init__()
        self.name = "钛迪ES2200门禁控制器协议"
        self.soi = b'\x7E'  # 1字节起始符
        self.eoi = b'\x0D'  # 1字节结束符
    
    def parse_packet(self, hex_string):
        """解析钛迪ES2200门禁控制器协议数据包"""
        # 将16进制字符串转换为字节流
...
if __name__ == "__main__":
    parser = TidiES2200Parser()
    
    # 示例数据包（需要替换为实际的钛迪ES2200协议数据包）
    example_packet = "7E100180480002000112340D"
    
    print("=== 钛迪ES2200门禁控制器协议解析示例 ===")
