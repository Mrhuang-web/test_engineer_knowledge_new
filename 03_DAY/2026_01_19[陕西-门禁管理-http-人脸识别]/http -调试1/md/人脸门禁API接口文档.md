...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/device/information?pass=12345678"
```
...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/getDoorSensor?pass=12345678"
```

### 2.11 信号输入设置
- **请求方式**：POST
- **请求URL**：/device/setSignalInput
- **参数说明**：
  | 参数名 | 类型 | 必传 | 说明 |
  | --- | --- | --- | --- |
  | pass | String | Y | 设备密码 |
  | config | String | Y | 信号输入配置 |
- **curl示例**：
```bash
curl -X POST "http://127.0.0.1:8090/device/setSignalInput" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "pass=12345678&config=CONFIG_DATA"
```

### 2.12 会议与关门告警设置
- **请求方式**：POST
- **请求URL**：/meetAndWarnSet
- **参数说明**：
  | 参数名 | 类型 | 必传 | 说明 |
  | --- | --- | --- | --- |
  | pass | String | Y | 设备密码 |
  | meetEnable | Int | N | 会议模式开关（0关，1开） |
  | meetFreeTime | Int | N | 会议空闲时间 |
  | doorWarnEnable | Int | N | 关门告警开关 |
  | doorCloseTime | Int | N | 关门时间 |
- **curl示例**：
```bash
curl -X POST "http://127.0.0.1:8090/meetAndWarnSet" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "pass=12345678&meetEnable=1&meetFreeTime=30&doorWarnEnable=1&doorCloseTime=5"
```
...
- **参数说明**：
  ...
  | --- | --- | --- | --- |
  | pass | String | Y | 设备密码 |
  | readDataEnable | Int | N | 读卡数据开关 |
  | readSector | Int | N | 读卡扇区 |
  | readBlock | Int | N | 读卡块 |
  | readShift | Int | N | 读卡偏移 |
  | readKeyA | String | N | 读卡密钥A |
  | wgOutType | Int | N | WG输出类型 |
- **curl示例**：
```bash
curl -X POST "http://127.0.0.1:8090/cardInfoSet" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "pass=12345678&readDataEnable=1&readSector=0&readBlock=1"
```
...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/device/status?pass=12345678"
```
...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/person/find?pass=12345678&id=-1"
```
...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/person/findByPage?pass=12345678&personId=-1&length=10&index=0"
```
...
curl -X POST "http://127.0.0.1:8090/face/update" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "pass=12345678&personId=PERSON_ID&faceId=FACE_ID&imgBase64=BASE64_IMAGE_DATA&isEasyWay=false"
```
...
- **curl示例**：
```bash
curl -X GET "http://127.0.0.1:8090/newFindRecords?pass=12345678&personId=-1&startTime=0&endTime=9999-12-31%2023:59:59&length=10&index=0"
```
