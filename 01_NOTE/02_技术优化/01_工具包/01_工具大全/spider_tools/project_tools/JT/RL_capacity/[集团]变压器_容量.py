...
def create_tah_by_device_signal(f=None, number=100, filename='d_tah_by_device_signal.sql',
                                begin_time="", end_time="", status_time=1, skip_hours=[2],
                                siteId="", roomId="",
    ...
    # 获取目前历史数据表中最大的id
    global d_tah_id_SN
    d_tah_id_SN = input_sql("d_tah_id_SN") + 1
    ...
    get_parmas = input_parma(siteId, roomId, deviceid, signalid, signal_number)
    device_rows_filter = input_sql("yl_device_name", **get_parmas)
    ...
    if mode == "1":
        if device_no == "1":
            ...
            device_ids_str = input_sql("device_ids_str", **get_parmas)

            # 第四步：构造 SignalID 列表字符串
            signal_ids_str = input_sql("yl_signal_ids_str", **get_parmas)
            print("signal_ids_str:", signal_ids_str)

            # 第五步：构建最终结果字段
            get_parmas.update({'device_ids_str': device_ids_str, 'signal_ids_str': signal_ids_str})
            signal_result = input_sql("signal_result", **get_parmas)
...
# sql判断
def input_sql(category: Optional[str], **args):
    if category == 'd_tah_id_SN':
        re = get_sql_result(sql='SELECT MAX(Id) FROM d_signalh')
        maxId = re[0][0] + 1 if re[0][0] is not None else 1
        return maxId
    if category == 'yl_device_name_filter':
        # yl_device_name = ["'1#工况环境'", "'中央空调主机'", "'2#工况环境'", "'系统参数'", "'1#一次侧机组（冷机/冷塔）'",
        #                   "'2#一次侧机组（冷机/冷塔）'", "'CDU1'", "'3#冷机'", "'1#冷机'", "'2#冷机'",
        #                   "'室外环境设备'"
        #                   ]
        yl_device_name = ["'智能电表'", "'高压配电柜'"]

        to_str = ",".join(yl_device_name)
        # todo 修改为控制指定类型
        device_rows = get_sql_result(
            sql=f"""SELECT DeviceID,DeviceName FROM m_device where SiteID IN ({args['siteId']})
                        AND DeviceName in ({to_str}) AND RoomID in ({args['roomId']})""")
        # todo 修改为不控制指定类型
        # device_rows = get_sql_result(
        #     sql=f"""SELECT DeviceID,DeviceName FROM m_device where SiteID IN ({args['siteId']})
        #                 AND RoomID in ({args['roomId']})""")
        return device_rows
    if category == 'device_ids_str':
        device_rows = get_sql_result(
            sql=f"""SELECT DeviceID,DeviceName FROM m_device where SiteID IN ({args['siteId']}) 
                        AND RoomID in ({args['roomId']})""")
        seen = set()
    ...
    if category == 'yl_signal_ids_str':
        ...
        # signal_list = ['092316', '092334', '092309', '092330', '001329', '001328', '001311'
        #     , '001312', '092330', '009331']
        signal_list = ['092330', '009331']
        # signal_list = ['013405']
        signal_ids_str = ",".join([f"'{id}'" for id in signal_list])
        print("signal_ids_str:", signal_ids_str)
        return signal_ids_str
    if category == 'signal_result':
        # todo 修改为指定测点
        sql = f'''SELECT SCID, SiteID, DeviceID, `Type`, SignalID, SignalNumber ,SignalName
                                 FROM m_signal
                                 WHERE
                                 SiteID IN ('{args['siteId']}')
                                 AND `Type` != 4
                                 AND SignalID IN ({args['signal_ids_str']})
                                 AND DeviceID IN ({args['device_ids_str']})
                                 AND SignalNumber IN ({args['signal_number']})'''
...
if __name__ == "__main__":
    ...
    siteId = '2028'
    roomId = '202801'  # 202801  202808  202812
