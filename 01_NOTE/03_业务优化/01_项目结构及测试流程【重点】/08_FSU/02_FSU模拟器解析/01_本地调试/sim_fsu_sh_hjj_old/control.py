        ...
        for req_device_node in  req_et.xpath('/Request/Info/DeviceList/Device'):
            ...
            if  device_id == None:    # 当为空，则返回该FSU所监控的所有设备的监控点的值；这种情况下，忽略IDs参数（即监控点ID列表)
                ...
                for device_obj in device_obj_list:
                    ...
                    for signal_obj in query_signals_by_device(fsuid,device_id):
                        _add_TSemaphore_Node(Device_Node, signal_obj)
            else:
                Device_Node = etree.SubElement(DeviceList, "Device")
                ...
                if signalid_list == []:        # 设备节点下的ID当为空，则返回该设备的所有监控点的值
                    device_tsemaphore_ID_list = [oo.id for oo in query_signals_by_device(fsuid,device_id)]
                    logger.info('device_tsemaphore_ID_list: %s' % device_tsemaphore_ID_list)
                    for tsemaphore_id in list(set(device_tsemaphore_ID_list)):
                        logger.info('tsemaphore_id: %s ' % tsemaphore_id)
                        if tsemaphore_id not in tmp_list:
                            signals_obj = queryTSemaphore(fsuid,device_id,str(tsemaphore_id))
                            logger.info('signals_obj: %s' % signals_obj)
                            if signals_obj != []:
                                tsemaphore_obj = signals_obj[0]
                                _add_TSemaphore_Node(Device_Node, tsemaphore_obj)
                                tmp_list.append(tsemaphore_id)
                else:
                    for id in req_et.xpath('/Request/Info/DeviceList/Device[@ID="%s"]/ID' % device_id):
                        ID = id.text
                        for tsemaphore_obj in querySignals(fsuid,device_id,ID):
                            sinalid_sianlnumber = '{}:{}'.format(ID,tsemaphore_obj.signalnumber)
                            logger.info('current signalNumber: %s' % sinalid_sianlnumber)
                            if sinalid_sianlnumber != None:
                                if sinalid_sianlnumber not in tmp_list:
                                    _add_TSemaphore_Node(Device_Node, tsemaphore_obj)
                                    tmp_list.append(sinalid_sianlnumber)
                # todo 废弃
                #for id in req_et.xpath('/Request/Info/DeviceList/Device[@ID="%s"]/ID' % device_id):
                #    ID = id.text
                #    for tsemaphore_obj in queryTSemaphore(fsuid,device_id,ID):
                #        sinalid_sianlnumber = '{}:{}'.format(ID,tsemaphore_obj.signalnumber)
                #        if sinalid_sianlnumber != None:
                #            if sinalid_sianlnumber not in tmp_list:
                #                _add_TSemaphore_Node(Device_Node, tsemaphore_obj)
                #                tmp_list.append(sinalid_sianlnumber)
                #        else:                               # ID当为空，则返回该设备的所有监控点的值。
                #            device_tsemaphore_ID_list = [oo.id for oo in query_signals_by_device(fsuid,device_id)]
                #            for tsemaphore_id in list(set(device_tsemaphore_ID_list)):
                #                if tsemaphore_id not in tmp_list:
                #                    _add_TSemaphore_Node(Device_Node, tsemaphore_obj)
...
def _add_TSemaphore_Node(parent_node,tsemaphore_obj):
    TSemaphore = tsemaphore_obj
    if TSemaphore != None:
        TSemaphore_Node = etree.SubElement(parent_node, "TSemaphore")
        TSemaphore_Node.set('Type', TSemaphore.type)
        TSemaphore_Node.set('ID', str(TSemaphore.signalsid))
        TSemaphore_Node.set('SignalNumber', TSemaphore.signalnumber.zfill(3))
        rand1 = str(float(random.randrange(1,100)))
        rand2 = str(float(random.randrange(1,100)))
        #TSemaphore_Node.set('MeasuredVal', TSemaphore.measuredval)
        #TSemaphore_Node.set('SetupVal', TSemaphore.setupval)
        if TSemaphore.measuredval in (None,''):
            TSemaphore_Node.set('MeasuredVal', rand1)
        else:
            TSemaphore_Node.set('MeasuredVal', TSemaphore.measuredval)
        TSemaphore_Node.set('SetupVal', rand2)
        #if TSemaphore.signalsid == config.METE_CODE:
        #   TSemaphore_Node.set('MeasuredVal', config.METE_VALUE)
        #   TSemaphore_Node.set('SetupVal', rand2)
        #else:
        #    TSemaphore_Node.set('MeasuredVal', rand1)
        #    TSemaphore_Node.set('SetupVal', rand2)
        #TSemaphore_Node.set('Status', TSemaphore.status)
        TSemaphore_Node.set('Status', '0')
        # TSemaphore_Node.set('Time', str(TSemaphore.time))
...
def on_SET_POINT(req_et):
    ...
    for device_id in device_id_list:
        TSemaphore_id_list = [li.get('ID') for li in req_et.xpath('/Request/Info/Value/DeviceList/Device[@ID="%s"]/TSemaphore' % device_id)]
        ...
        for TSemaphore_id in TSemaphore_id_list:
            ...
            req_tsemaphore_node_list =req_et.xpath('/Request/Info/Value/DeviceList/Device[@ID="%s"]/TSemaphore[@ID="%s"]' % (device_id,TSemaphore_id))
    ...
    for deviceid in deviceid_list:
        ...
        for threshold_node in threshold_node_list:
            ...
            if threshold_obj ==None:
                threshold_obj = Threshold(deviceid=deviceid,thresholdid=thresholdid,type=type,signalnumber=signalnumber,
                                          threshold=threshold,alarmlevel=alarmlevel,nmalarmid=nmalarmid)
                session.add(threshold_obj)
                session.commit()
            else:
                session.query(Signals).filter_by(fsuid = fsuid).filter_by(deviceid = deviceid).filter_by(signalsid=thresholdid).update({"type": type,
                                                                "deviceid": deviceid,
...
def on_GET_FSUINFO(req_et):
    ...
    if fsuobj == None:
        ...
        etree.SubElement(TFSUStatus, "CPUUsage").text = fsuobj.tfsustatus_cpuusage
        etree.SubElement(TFSUStatus, "MEMUsage").text = fsuobj.tfsustatus_memusage
        etree.SubElement(TFSUStatus, "HardDiskUsage").text = fsuobj.tfsustatus_harddiskusage
        __add_success_result_node(Info)
    return etree.tostring(Response, pretty_print=True, xml_declaration=True, encoding="utf-8")
...
def on_SET_DEV_CONF_DATA(req_et):
    ...
    for deviceid in deviceid_list:
        ...
        for sinals_node in sinals_node_list:
            ...
            if sinals_obj ==None:
                signalsobj = Signals(signalsid=signalsid,deviceid=deviceid,type=type,signalname=signalname,signalnumber=signalnumber,
                                     alarmlevel=alarmlevel,thresbhold=thresbhold,nmalarmid=nmalarmid)
                session.add(signalsobj)
                session.commit()
            else:
                session.query(Signals).filter_by(fsuid=fsuid).filter_by(deviceid=deviceid).filter_by(signalsid=signalsid).update({"type": type,
                                                                "deviceid":deviceid,
...
def on_GET_STORAGERULE(req_et):
    ...
    DeviceList = etree.SubElement(Values, 'DeviceList')
    for deviceid in deviceid_list:
        tsignalmeasurementid_node_list = req_et.xpath('/Request/Info/DeviceList/Device[@ID="%s"]/TSignalMeasurementId' % deviceid)
        Device_Node = etree.SubElement(DeviceList, 'Device')
        Device_Node.set('ID',deviceid)
    ...
    for deviceid in deviceid_list:
        ...
        for tstoragerule_node in tstoragerule_node_list:
            ...
            if device_obj == None:
                ...
                if storage_obj ==None:
                    storage_obj = Storage(storageid=storageid,fsuid=fsuid,deviceid=deviceid,type=type,signalnumber=signalnumber,
                                          absoluteval=absoluteval,relativeval=relativeval,storageinterval=storageinterval,
                                          storagereftime=storagereftime)
                    session.add(storage_obj)
                    session.commit()
                else:
                    session.query(Storage).filter_by( fsuid = fsuid ).filter_by( deviceid = deviceid ).filter_by( storageid = storageid ).update({
                                                                      "type": type,
                                                                      "fsuid": fsuid,
                                                                      "deviceid":deviceid,
                                                                      "signalnumber": signalnumber,
