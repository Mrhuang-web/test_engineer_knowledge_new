...
class DeviceService:
    ...
    def reset_device():
        """重置设备到初始状态"""
        from data import device_status, person_data, face_data, record_data
    ...
    def set_signal_input(config):
        """信号输入设置"""
        if not config:
            return base_response(0, False, 'config参数异常', ERROR_CODES['OLD_PASSWORD_ERROR'])
